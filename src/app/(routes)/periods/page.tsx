    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 rounded-[32px] border border-[var(--border)] bg-[var(--surface)] p-8 shadow-sm md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex gap-3">
          <Link href="/api/export/historical-periods.csv" className="inline-flex rounded-full border border-[var(--border)] px-5 py-2.5 text-sm font-medium">
            CSV エクスポート
          </Link>
      ...
      <form className="grid gap-4 rounded-[32px] border border-[var(--border)] bg-white/80 p-6 shadow-sm md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/periods" className="inline-flex whitespace-nowrap rounded-full border border-[var(--border)] px-5 py-2.5 text-sm font-medium">
            リセット
          </Link>
          <p className="ml-auto whitespace-nowrap text-sm text-[var(--muted)]">検索結果：{periods.length}件</p>
        </div>
      </form>
