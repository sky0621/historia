...
export function buildPersonTagLinksCsv() {
  const rows = sqlite
    .prepare(
      `SELECT
         p.id AS person_id,
         ...
         t.id AS tag_id,
         t.name AS tag_name
       FROM person_tag_links ptl
       INNER JOIN persons p ON ptl.person_id = p.id
       INNER JOIN tags t ON ptl.tag_id = t.id
       ORDER BY p.id, t.id`
    )
    .all() as Array<Record<string, unknown>>;

  return toCsv(rows, ["person_id", "person_name", "tag_id", "tag_name"]);
}
...
export function buildDynastyTagLinksCsv() {
  const rows = sqlite
    .prepare(
      `SELECT
         d.id AS dynasty_id,
         ...
         t.id AS tag_id,
         t.name AS tag_name
       FROM dynasty_tag_links dtl
       INNER JOIN dynasties d ON dtl.dynasty_id = d.id
       INNER JOIN tags t ON dtl.tag_id = t.id
       ORDER BY d.id, t.id`
    )
    .all() as Array<Record<string, unknown>>;

  return toCsv(rows, ["dynasty_id", "dynasty_name", "tag_id", "tag_name"]);
}
