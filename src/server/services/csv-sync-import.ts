...
export function importCsvSync(targetType: CsvSyncImportTarget, rawCsv: string): CsvSyncImportResult {
  switch (targetType) {
    ...
    case "person-tag-links":
      return importPersonTagLinksCsv(rawCsv);
...
function importPersonTagLinksCsv(rawCsv: string): CsvSyncImportResult {
  const parsed = parseCsv(rawCsv);
  assertHeaders(parsed.headers, ["person_id", "person_name", "tag_id", "tag_name"]);

  return db.transaction((tx) => {
    const existingLinks = tx.select().from(personTagLinks).all();
    const personOptions = tx.select().from(persons).all();
    const tagOptions = tx.select().from(tags).all();
    const personNameById = new Map(personOptions.map((item) => [item.id, item.name]));
    const tagNameById = new Map(tagOptions.map((item) => [item.id, item.name]));
    const existingKeys = new Set(existingLinks.map((item) => `${item.personId}:${item.tagId}`));
    const csvKeys = new Set<string>();
    let createdCount = 0;
    let updatedCount = 0;
    ...
    for (const row of parsed.rows) {
      ...
      if (csvKeys.has(key)) {
        throw new Error(`row ${row.rowNumber}: person_id=${personId}, tag_id=${tagId} が CSV 内で重複しています`);
      }
      csvKeys.add(key);
      ...
      if (!existingKeys.has(key)) {
        tx.insert(personTagLinks).values({ personId, tagId }).run();
        createdCount += 1;
      } else {
        updatedCount += 1;
      }
    ...
    for (const link of existingLinks) {
      const key = `${link.personId}:${link.tagId}`;
      if (!csvKeys.has(key)) {
        tx
          .delete(personTagLinks)
          .where(and(eq(personTagLinks.personId, link.personId), eq(personTagLinks.tagId, link.tagId)))
          .run();
      }
    }

    const deletedCount = existingLinks.filter((link) => !csvKeys.has(`${link.personId}:${link.tagId}`)).length;

    return {
      targetType: "person-tag-links" as const,
      totalRows: parsed.rows.length,
      createdCount,
      updatedCount,
      deletedCount
