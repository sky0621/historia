...
export function createRegion(input: RegionInsert, parentRegionId: number | null) {
  const result = db.insert(regions).values({ ...input, parentRegionId }).run();
  return Number(result.lastInsertRowid);
}

export function updateRegion(id: number, input: Omit<RegionInsert, "id">, parentRegionId: number | null) {
  db.update(regions).set({ ...input, parentRegionId }).where(eq(regions.id, id)).run();
}

export function deleteRegion(id: number) {
  db.update(regions).set({ parentRegionId: null }).where(eq(regions.parentRegionId, id)).run();
